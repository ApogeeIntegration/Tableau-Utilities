/*
   Copyright [2022] [APOGEE Integration]
   All Rights Reserved.
...
function findParameter() {
  // finds user inputed parameter, shows play pause button, and runs loop
  getInputValues();
...
function paramLoop() {
  // checks for user input error and runs each loop
  $('#play').show();
  ...
  if (
    global.parameter.allowableValues.type == 'range' &&
    global.parameter.dataType != 'date' &&
    global.parameter.dataType != 'date-time'
  ) {
    // range + number case
  ...
  } else if (global.parameter.dataType == 'date' && global.parameter.allowableValues.type == 'range') {
    // range + date case
  ...
  } else if (global.parameter.dataType == 'date-time' && global.parameter.allowableValues.type == 'range') {
    // range + date-time case
    global.parameter.changeValueAsync(global.min);
...
function getParameterMinMax() {
  // extracts maximum and minimum values of a parameter (If they exist)
  getInputValues();
  ...
  tableau.extensions.dashboardContent.dashboard.getParametersAsync().then((para) => {
    ...
    if (global.parameter.dataType == 'date-time' && minval != null) {
      // tableau outputs dates in non-standard format, converts to javascript format
    ...
    if (global.parameter.allowableValues.type != 'list') {
      if (minval != null) $('#max')[0].max = maxval;
      $('#min')[0].max = maxval;
      if (maxval != null) $('#max')[0].min = minval;
      $('#min')[0].min = minval;
    }

    // shows and hides relevant input boxes, executed on select
    if (global.parameter.allowableValues.type == 'list') {
      $('#stepSizeDiv').collapse('hide');
      ...
      if (global.parameter.dataType == 'date-time' && global.parameter.allowableValues.type != 'list') {
        ...
        for (var i = 0; i < global.datetimeUnits.length; i++) {
          $('#timeUnits').append(
            "<option value='" + global.datetimeUnits[i] + "'>" + global.datetimeUnits[i] + '</option>'
          );
        }
        $('#timeUnit').collapse('show');
      } else if (global.parameter.dataType == 'date' && global.parameter.allowableValues.type != 'list') {
...
function playPause() {
  // allows play pause button to work
...
function rangeLoop() {
  // creates interval that iterates parameter for user specified time
  if (global.time) {
...
function listLoop() {
  ...
  global.interval = setInterval(() => {
    global.parameter
      ...
      .then((val) => {
        // inconsitency without the use of async function
        $('.results').html(val.value);
      });
...
function rangeDateLoop() {
  // loop for standard date parameters
...
function removeOptions(selectElement) {
  // removes options from relevant
  var i,
    L = selectElement[0].options.length - 1;
  for (i = L; i >= 0; i--) {
...
function checkInputErrors() {
  // checks for various user input errors and displays modal right afterwards (may need work as css doesnt like rapid opening and closing of modals)
